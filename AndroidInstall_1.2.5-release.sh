#!/bin/bash
# AndroidInstall_1.2.5-release.sh
...
# log all system variables at script execution; kill script if signs of race attack are present
...
build="0353"; scriptVersion=1.2.5-release; author="Nikolas A. Wagner"; license="GNU GPLv3"
...
updateIP(){
	if ! adb -d shell curl -V >/dev/null 2>&1; then
		deviceIP="curl unavailable"
		deviceLOC="curl unavailable"
		return
	elif ! adb -d shell curl "www.google.com" >/dev/null 2>&1; then
		deviceIP="Device can't connect to internet"
		deviceLOC="Device can't connect to internet"
		return
	else
		# device has the minimum requirements for IP location feature, so try to updateIP
	...
	if [ "$usrIP" = "" ]; then
		usrIP="unexpected error"
	fi

	if [ "$devIP" = "" ]; then
		deviceIP="unexpected error"
		deviceLOC="unexpected error"
	fi
}
...
update_IPdata(){
	...
	# get IP addresses, then use them as inputs for the IPdata websites
...
parse_IPdata(){
	...
	# give 'bounce_' functions a way to lookup the data from file using parameter expansion
	readXML(){
		IFS=\>
	...
	# check each line one-by-one to find/bounce the desired information
	while readXML; do
		bounce_usrIPdata
...
INIT(){
	...
	testVar1=""
	if ! (echo ${testVar1,,}); then
		if [ "$verbose" = 1 ] || [ "$verbose" = 2 ]; then
			echo "testing for bash v4+ parameter expansion compatibility"; echo
		fi
...
gitConfigs(){
	...
	if ! (CMD_gitGet) && wait; then
		if ! ping -t 1 "www.google.com"; then
			scriptTitle="$scriptTitleDEF"; warnNoFi="true"

			if [ "$verbose" = 1 ] || [ "$verbose" = 2 ]; then
				echo
				echo "Cannot connect to git; the terminal has no connection to the internet!"
				sleep 2
			fi
...
getOBB(){
	...
	# if OBB is still invalid after the if statements, make user try again
...
installAgainPrompt(){
	...
	if [ "$APKname" = "" ] || [ "$APKname" = "tool" ] || [ "$APKname" = "tools" ]; then
		noInstall="true"
...
# check power state of device (for locked or sleeping status)
adbWAKE(){
	devLock=""; devSleep=""

	devLock=$(adb -d shell dumpsys power | grep 'mHoldingWakeLockSuspendBlocker')
	devSleep=$(adb -d shell dumpsys power | grep 'mHoldingDisplaySuspendBlocker')

	if [ "$verbose" = 2 ]; then
		echo "$devLock"; echo "$devSleep"; echo
	fi

	while [[ ! "$devLock" == *"true" ]] && [[ ! "$devSleep" == *"true" ]]; do
		echo "Please wake your device! Press any key to continue"; echo
		read -n 1 -s -r -p '' && perl -e 'use POSIX; tcflush(0, TCIFLUSH);'
	...
	if (CMD_communicate); then
		...
		{ sleep 3; printf "        Ensure only one device is connected!"; } & { 
			until (CMD_communicate); do
				if [ "$qMode" = "false" ]; then waiting; fi; deviceConnect="true"
			done
